...
- **Only `23505` is mapped — `23503` (foreign key violation) is not.** So a bad FK target surfaces as a 500, not a 404. That is why services validate a referenced row exists *before* the insert/update rather than letting the constraint fire: `createRoute`/`updateRoute` call `findSubjectBySlug` first, `setMySubjects` calls `countExistingSubjects`. Follow that pattern for any new FK a client can supply.
...
- **An unverified login is `EMAIL_NOT_VERIFIED` (403), not `INVALID_CREDENTIALS`, and it re-sends the code on the way out.** The local strategy passes a third `info` argument carrying the code; `login` in `auth.controllers.js` branches on it, calls `resendVerifyCode` and throws. Without that `info` the controller falls through to its `??` and answers *byte-for-byte the same as a wrong password* — which is what it did, leaving anyone who registered and never confirmed stuck being told their password was wrong, with no route in the UI to the code modal. The re-send is there because the registration code expires in 15 minutes, so opening the verify modal with a dead one is useless; it is wrapped in try/catch so a mail failure cannot mask the real reason for the rejection. **Precedence matters and is tested: a wrong password on an unverified account still answers `INVALID_CREDENTIALS`**, because bcrypt is compared before the verified check. 403 and not 401 because the credentials *are* correct — it is "I know who you are and you may not yet", not "who are you?".
- **`loginSchema` is deliberately *not* `authSchema`.** It checks a valid email and a non-empty password, nothing more. Reusing the registration schema made login validate the password *format*: tightening the policy would have locked out existing users with a 400 about special characters rather than a clean auth failure, and it handed the policy to anyone probing the endpoint. Whether the password is right is bcrypt's answer, not zod's.
...
- **`googleCallback` redirects to `${CLIENT_URL}/app/home`, not to `CLIENT_URL`.** The bare version dropped the user on the logged-out landing, which paints no avatar and does not mount the session-driven onboarding modal — that lives only in `app/app/layout.js`, under `/app/*`. The symptom was signing in with Google and appearing to have neither a profile photo nor an onboarding step, when the database had both the Google photo and the pending flag correctly set. **Anything that opens a session by redirect has to land inside the guarded tree**, or it looks like the sign-in silently did nothing. The `failureRedirect` goes to `/?error=google` for the same reason: `/login` is not a route in this frontend.
- **The Google photo needs no special handling downstream.** `auth.strategys.js` stores `profile.photos?.[0]?.value` in `users.image`, and `urlOfReading` returns anything starting with `http` untouched rather than trying to sign it against S3 (`isStorageKey`). Linking Google to an existing local account uses `image = COALESCE(image, $2)` on purpose: an avatar the user uploaded deliberately outranks the one Google supplies.
...
**A route has exactly one subject, not a list of topics.** The `route_topics` join table was dropped; `routes.subject_slug` is `NOT NULL` with `ON UPDATE CASCADE ON DELETE RESTRICT` — deleting a catalog subject that routes use must fail rather than orphan them. `createRouteSchema.subject` is required (a route with no subject could never enter the feed), `updateRouteSchema.subject` is optional. Responses carry a single `subject: { slug, name }` object, never a `subjects` array.

**The slug IS the identifier — there are no subject uuids anywhere** (migration 19). `subjects.slug` is the `TEXT PRIMARY KEY` and both FK columns (`routes.subject_slug`, `user_subject_preferences.subject_slug`) point at it. That was the whole point: the catalog is fourteen fixed rows that nobody creates, renames or deletes, `slug` was already `NOT NULL UNIQUE`, and the frontend keys five separate lookups off it. Before this, a client that wanted to save an interest had to fetch the whole catalog just to translate `matematicas` into a uuid. Now `{ "subjects": ["matematicas", "fisica"] }` is the request.

Two consequences worth knowing. `INSERT`/`UPDATE` can't `JOIN`, but their `RETURNING` now yields the slug — which is the key — so only the accented `name` is missing, and `findSubjectBySlug` supplies it as a by-product of the existence check it was already doing. That killed `withTopicColumns` and the extra round-trip `updateRoute` used to make. And `listPublishedRoutes` filters on `r.subject_slug` directly instead of reaching through the catalog.

**The catalog is exactly fourteen subjects, flat** (migration 16): `matematicas`, `biologia`, `quimica`, `fisica`, `historia`, `ciencias-sociales`, `lengua-literatura`, `idiomas`, `filosofia`, `arte`, `musica`, `programacion`, `ingenieria`, `economia`. It used to be ~95 rows grouped by a `discipline` column; both the extra rows and the column are gone — the fourteen are distinct subjects and none is a sub-subject of another. **The slug is the contract**, because the frontend resolves a subject's banner, icon and card tint from it (`SUBJECTS_LABELS`/`CATEGORY_ICONS`/`--subject-<key>`), and an unknown slug renders with none of the three and no error. Adding a subject means adding it on both sides. Changing a slug's *value* is the genuinely dangerous edit — every one of those lookups fails silently — which is what `ON UPDATE CASCADE` is there for on the database side.

`subjects.name` carries accents (`Matemáticas`, `Biología`) so a client can paint it directly; the slug never does. The wildcard `otros` was deleted with the rest: `createRouteSchema.subject` is required, so nothing needs a fallback. **Migration 11 still depends on `otros` existing** for its backfill — it runs before 16, so a rebuild from scratch is fine, but do not remove it from migration 10.

`listSubjects` orders by `name`. The curated order `/explore` uses for its grid (sciences → humanities → applied) lives in the frontend, not here.

**`activity_events.target_id` is not a subject.** It is the id of the route, lesson or quiz an event refers to, and it was called `subject_id` until migration 19 renamed it — with `subjects` now meaning the catalog, the old name sat one underscore away from `subject_slug` and meant something entirely unrelated. `recordActivity` and everything that calls it take `targetId`.
...
`user_subject_preferences` is what feeds the `preference` term. `PUT /users/me/subjects` — **`PUT`, not `POST`**, because it is a full idempotent replacement, so the same endpoint serves both the initial onboarding screen and later edits from settings. Limits live in `SUBJECT_SELECTION` (`{ min: 1, max: 5 }`) in `users.services.js`, and `users.schemas.js` imports it so the two cannot drift. That schema also reuses `subjectSlug` from `routes.schemas.js` rather than declaring its own — it is the same field, and two definitions of it would diverge.

- **Slugs are deduplicated before being counted** against `countExistingSubjects`. `[a, a, b]` has three elements but two subjects; comparing 2 against 3 would reject a valid request.
...
- `GET /users/me` returns `subjects`, `onboardingCompleted` and `onboardingCompletedAt` alongside the gamification stats. `PATCH /users/me` updates the display name only.
- `user_subject_preferences` has **no `weight` column**. The old table carried `weight SMALLINT DEFAULT 1` that nothing ever read or wrote — the `preference` term is a flat 0/1 — so migration 19 dropped it rather than carry a field that advertised a feature that does not exist.

**`PUT /users/me/onboarding` (`{ name, subjects }`) is the sign-up step, and the only thing that ever sets `onboarding_completed_at`.** It exists because `POST /auth/register` takes email and password only, so `createLocalUser` inserts a row with **`name` NULL** — and the client draws its avatar from the initials of the name, so those users had neither name nor avatar. Name and interests go in one transaction because the modal collects them together and half of it is useless. `PUT /users/me/subjects` deliberately **no longer marks onboarding**: if editing interests from settings still marked it, a user with no name would complete onboarding through the back door.
...
17. `1785862217307_drop-route-comments.sql` — quita `route_comments`; los comentarios salieron del producto en favor de las valoraciones por estrellas.
18. `1785875394984_users-username.sql` — `users.username`, el handle con el que el front enruta los perfiles.
19. `1785975856497_rename-topics-to-subjects.sql` — **`topics` pasa a `subjects` y el `slug` pasa a ser la clave primaria**, con lo que el uuid de materia desaparece de la base y del contrato. `routes.topic_id` → `routes.subject_slug` y `user_topic_preferences` → `user_subject_preferences`, las dos FK con `ON UPDATE CASCADE` porque una clave natural sí se puede querer renombrar. Se aprovecha para tirar `weight`, que no leía ni escribía nadie. También renombra **`activity_events.subject_id` → `target_id`**: nunca fue una materia, y con `subjects` ocupando la palabra el nombre viejo era una trampa permanente. Ese `RENAME COLUMN` **no obliga a recrear `uq_activity_events_xp_once`** — Postgres guarda las columnas del índice por número de atributo, así que la definición lo sigue sola.
